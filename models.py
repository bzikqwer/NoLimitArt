...
class PhotoUploadField(FileUploadField):
    ...
    def pre_validate(self, form):
        if self.data:
            ...
            folder = os.path.join(self.base_path, form.user.username)
...
class UsersView(ModelView):
    ...
    form_extra_fields = {
        'photo': PhotoUploadField('Photo', base_path='static/images', relative_path='images/')
    }
    create_template = 'create_user.html'

    def on_model_change(self, form, model, is_created):
        """Override method to update photo field with the uploaded file path."""
        if form.photo.data:
            model.photo = form.photo.path
